 ...
 * 1代表true
 */
public class PgBooleanQueryParser implements QueryParser {
    @Override
    public void parse(StringBuilder selSql, SqlHelper.FromBuilder fromSql, StringBuilder whereSql, StringBuilder groupSql, Map<String, Object> params,
        ...
        if (value != null) {
            if (value) {
                whereSql.append(" and ").append(alias).append(".").append(sqlFieldName).append(" = true ");
            } else {
                whereSql.append(" and (").append(alias).append(".").append(sqlFieldName).append(" = false or ").append(alias).append(".").append(sqlFieldName).append(" is null) ");
            }
        }
    }
}
